...
try {
  ...
  if (process.argv.includes("--live")) {
    ...
    assert.equal(research.result?.structuredContent?.answer, liveResult?.run?.result?.assistant_text);
    const toolNames = (liveResult?.run?.tool_calls || []).map((event) => event.tool_name);
    ...
    if (process.argv.includes("--workspace")) {
      ...
      assert.ok(
        (workspaceResult?.run?.tool_calls || []).some((event) => event.tool_name === "read"),
    ...
    if (useLifecycle) {
      ...
      assert.equal(waited.result?.structuredContent?.answer, waitedResult?.run?.result?.assistant_text);
